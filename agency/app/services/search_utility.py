...
def check_max_number_of_cache(path: Path, type, max_number_of_cache: int = 10):
    ...
    if len(os.listdir(path)) > max_number_of_cache:
        ctime_list = [
            (os.path.getctime(path / file), file) for file in os.listdir(path)
        ]
        ...
        def wrapper(*args, **kwargs):
            ...
            if config.get("cache").get("is_enable").get(cache_type):
                ...
                if check_result_cache_exists(cache_path, cache_hash, cache_type):
                    result = load_result_from_cache(cache_path, cache_hash, cache_type)[
                        "result"
                    ]
                else:
                    ...
                    check_max_number_of_cache(
                        cache_path,
                        cache_type,
                        config.get("cache").get("max_number_of_cache"),
                    )
            else:
                result = func(*args, **kwargs)

...
parser = argparse.ArgumentParser(
    description="Set up WANDB project and LLM configurations."
)
...
parser.add_argument(
    "--llm",
    type=str,
    choices=["gpt-3.5-turbo", "gemma-7b-it", "mixtral-8x7b-32768"],
...
for idx, row in enumerate(all_samples.iterrows()):
    dspy_example = dspy.Example(
        {"question": row[1]["Question"], "answer": t_2_i[row[1]["route_option"]]}
...
confusion_data = (
    log_df.groupby(["llm", "answer", "prediction"]).size().unstack(fill_value=0)
)
